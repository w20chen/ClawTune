...
- EdgeKappaKB is exposed as the fourth independent predictor. Validate live concurrent prediction/completion delivery, restart/drain durability, empirical-duration composition and paired call-level quality on Linux. Confirm highest-bucket censor bounds from source telemetry before using censored feedback. Establish edge-specific gain over fixed/shared-child weights on task-paired splits; integration alone does not establish accuracy.
...
- KB ingestion parity acceptance: run `$env:PYTHONPATH='services/sidecar/src'; python -m pytest services/sidecar/tests tests -q -rs --basetemp .runtime/pytest-kb-validation` on Windows; on Linux use `PYTHONPATH=services/sidecar/src python3 -m pytest services/sidecar/tests tests -q -rs`. Linux collector/POSIX cases and a fresh online benchmark remain required to compare CPU interval deltas, retained duration, and environment memory against recorded replay. Historical traces without interval evidence cannot supply these missing labels. The attempted path `services/sidecar/tests/test_tool_resource_sdk.py` does not exist; SDK regression tests are `test_tool_resource_sdk_counts.py` and `test_tool_resource_sdk_concurrency.py`.

- Fourth-KB live acceptance: `python3 scripts/clawtune.py benchmark --benchmark swe-rebench --sample 3 --parallelism 3` requires Linux, Docker, OpenClaw and provider credentials; it cannot run on this Windows host. Check `prediction.edge_kappa` before tools, admitted clause feedback, four-KB checkpoints and restart, and separate duration coverage/error from unsupported resources. Repeat `PYTHONPATH=services/sidecar/src python3 -m pytest services/sidecar/tests tests -q -rs` on Linux for the skipped POSIX/native-parser cases.
- The attempted targeted pytest command ending in `tests/test_offline*.py` could not collect on PowerShell because pytest received that literal nonexistent path; offline integration tests are in `tests/test_demo_workflows.py` and are included in the root suite.
- Review validation: `npm test` in `packages/clawtune-plugin` could not start because PowerShell blocks `npm.ps1`; use `npm.cmd test` on this host.
- Review reproduction prerequisite: inline `python -` probes importing `clawtune_sidecar` cannot run without `$env:PYTHONPATH='services/sidecar/src'` on this checkout (`ModuleNotFoundError`).
