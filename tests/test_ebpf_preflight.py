...
from tools.check_ebpf import run_preflight
...
def test_preflight_reports_semantic_success(monkeypatch) -> None:
    monkeypatch.setattr("tools.check_ebpf.shutil.which", lambda name: f"/usr/bin/{name}")

    report = run_preflight(_fake_telemetry())

    assert report["ebpf_ready"] is True
