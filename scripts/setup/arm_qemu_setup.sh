...
usage() {
  cat <<'EOF'
Usage:
  sudo bash scripts/setup/arm_qemu_setup.sh [install|status|check] [--dry-run] [--smoke-image IMAGE]

Commands:
  install   Enable amd64/x86_64 Docker image execution on ARM hosts.
  status    Print host Docker/binfmt status.
  check     Run an amd64 smoke container and verify QEMU works.
...
install_amd64_emulation() {
  ...
  if run_cmd docker run --privileged --rm tonistiigi/binfmt --install amd64; then
    return 0
  fi
...
run_smoke_check() {
  ...
  if output="$(docker run --rm --platform linux/amd64 "${SMOKE_IMAGE}" /bin/sh -lc 'uname -m && echo arm-qemu-ok' 2>&1)"; then
    :
  else
    printf '%s\n' "${output}" >&2
    return 1
  ...
  if ! grep -Eq 'x86_64|amd64' <<<"${output}"; then
    echo "Smoke check did not report amd64 userspace." >&2
    return 1
  fi
  if ! grep -q 'arm-qemu-ok' <<<"${output}"; then
    echo "Smoke check marker missing from output." >&2
    return 1
  fi
}
