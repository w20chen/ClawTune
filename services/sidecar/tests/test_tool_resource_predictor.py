...
from clawtune_sidecar.predictors.tool_resource import (
...
def test_ebpf_workload_result_preserves_masked_lookup_diagnostic() -> None:
    workload = tool_resource_predictor._ebpf_workload_result(
        {
...
def test_predictor_retries_ebpf_after_container_id_arrives(tmp_path: Path) -> None:
...
def test_ebpf_clause_identity_matches_online_prediction(tmp_path: Path, monkeypatch) -> None:
...
def test_shared_snapshots_reuse_same_repo_evidence_but_isolate_other_repos() -> None:
    ...
    runtime.observe_completed_call(
        CompletedCall(
            ...
            sampled_peak_rss_bytes=32 * 1024**2,
...
def test_ebpf_loader_uses_native_sdk_artifact_validation(tmp_path: Path) -> None:
    ebpf = tmp_path / "invalid-ebpf.json"
    ebpf.write_text(
        json.dumps(
            {
                "version": 2,
    ...
    assert predictor.report.ebpf_traces_loaded == 0
...
def test_commandless_repo_tool_name_learning_is_causal_and_survives_snapshot(
    ...
    kb.observe_completed_call(
        CompletedCall(
            ...
            cpu_peak_cores=0.5,
            cpu_peak_cores_eligible=True,
            sampled_peak_rss_bytes=40 * 1024**2,
            sampled_peak_rss_eligible=True,
...
def test_sidecar_response_includes_kv_ttl_cost_key(tmp_path: Path) -> None:
    ...
    None (no eBPF data available for a bucket prediction)."""
    trace = tmp_path / "trace.jsonl"
    _write_trace(trace)
    state = build_state(
        SidecarConfig(
