...
from clawtune_sidecar.contracts.models import ToolBeforeRequest, ToolCompletedEvent
from clawtune_sidecar.monitoring.tool_runtime import ToolRuntimeSample
from clawtune_sidecar.telemetry.cgroup_resource import (
...
def test_write_cgroup_resource_writes_independent_artifact(tmp_path: Path) -> None:
    ...
    # cgroup memory.current is not process RSS and must not be exported under
    # an RSS field even when a legacy sample carried that value.
    assert data["memory_rss_peak_bytes"] is None
    ...
    assert data["memory_source"] == "unavailable"
    assert data["disk_source"] == "cgroup-v2-io.stat"
    assert data["network_source"] == "unknown-legacy-network-source"
    assert data["fallback_used"] is True
    assert data["sampling_coverage_ms"] == 611
    assert data["attribution_source"] == "shared-sandbox-container"
    assert "network independence is unknown" in data["independence"]
...
def test_verified_host_pid_scope_helpers() -> None:
    from clawtune_sidecar.api.app import (
