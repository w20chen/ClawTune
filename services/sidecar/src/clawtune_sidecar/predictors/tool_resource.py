...
class ToolResourceLoadReport:
    ebpf_traces_seen: int
    ebpf_traces_loaded: int
...
class ToolResourcePredictor:
    ...
    def from_traces(
        ...
        *,
        ...
        ebpf_trace_paths: Iterable[Path],
        buckets: LatencyBuckets,
        repo: str = "openclaw",
        artifact_dir: Path | None = None,
        container_executable: str = "docker",
        ...
        for path in ebpf_paths:
            try:
                artifact = _read_ebpf_artifact(path)
                observations.extend(_ebpf_observations(artifact, fallback_repo=repo))
                ebpf_loaded += 1
        ...
        predictor = cls(
            ...
            report=ToolResourceLoadReport(
                ...
                ebpf_traces_loaded=ebpf_loaded,
    ...
    def finish_execution(
        ...
        *,
        ...
        if incomplete_reason is not None and (
            ...
            raise ValueError(
                "incomplete eBPF finalization cannot carry launcher exit status"
            )
        with self._execution_lock:
        ...
        if run is None:
            return self._telemetry_by_execution_id.get(
                ...
                ExecutionTelemetrySummary(
                    ...
                    started=False,
                    status="unavailable",
                    unavailable_reason="no_active_ebpf_run",
                ),
            )
        replay_execution = (
            "incomplete"
        ...
        workload_result = _ebpf_workload_result(
            raw_result,
            exit_code=exit_code,
            signal=signal,
...
def _read_ebpf_artifact(path: Path) -> dict[str, Any]:
    return _load_valid_artifact(path)


def _ebpf_observations(
    ...
    *,
    ...
    for call in artifact.get("calls", []):
        ...
        for clause in clauses:
            completed = _ebpf_completed_call(repo, clause)
            if completed is not None:
                calls.append(completed)
    return calls


def _ebpf_completed_call(repo: str, row: Any) -> CompletedCall | None:
...
def _ebpf_workload_result(
    ...
    *,
    ...
    """Build the bounded live-result envelope consumed by eBPF.
    ...
    result = _ebpf_result_text(raw_result)
    stderr = _ebpf_named_text(raw_result, "stderr")
    return {
        "exit_code": exit_code,
        "signal": signal,
        "ok": (
        ...
        "result": _bounded_ebpf_text(result),
        "stderr": _bounded_ebpf_text(stderr),
    }


def _ebpf_result_text(value: Any) -> str:
    ...
    return _ebpf_result_text(nested) if nested is not value else ""


def _ebpf_named_text(value: Any, key: str) -> str:
    if not isinstance(value, Mapping):
        return ""
    details = value.get("details")
    for container in (details, value):
...
def _bounded_ebpf_text(value: str, limit: int = 65_536) -> str:
        ...
        compact.append(
            {
                ...
                # Map from the eBPF artifact's actual keys: the clause row
                ...
                # The eBPF eBPF collector does not monitor network today;
...
def _discover_artifact_call_files(artifact_dir: Path) -> list[Path]:
    """Return eBPF call artifact paths found in *artifact_dir*.
