...
def _repo_root() -> Path | None:
    ...
    try:
        import clawtune_sidecar

        package_file = Path(clawtune_sidecar.__file__).resolve()
...
def doctor_main() -> None:
    ...
    try:
        ...
        report["clawtune_sidecar"] = {
            "importable": True,
            "version": getattr(clawtune_sidecar, "__version__", "unknown"),
        }
    except ImportError:
        report["clawtune_sidecar"] = {"importable": False}
    ...
    if not report["cgroup_v2"]:
        ...
        ok = False
    if not report["clawtune_sidecar"]["importable"]:
        print("WARNING: clawtune_sidecar is not importable; the sidecar is not installed.", file=sys.stderr)
        ok = False
