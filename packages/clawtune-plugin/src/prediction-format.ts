...
const targets: LoadTarget[] = [
  "duration_ms", "cpu_time_seconds", "cpu_avg_cores", "cpu_peak_cores", "sampled_peak_rss_bytes",
  "memory_total_peak_bytes", "memory_extra_peak_bytes",
];
const labels: Record<LoadTarget, string> = {
  duration_ms: "Duration", cpu_time_seconds: "CPU time", cpu_avg_cores: "CPU average",
  cpu_peak_cores: "CPU peak", sampled_peak_rss_bytes: "Sampled RSS peak",
  memory_total_peak_bytes: "Memory total", memory_extra_peak_bytes: "Memory extra",
};
...
function formatEstimate(target: LoadTarget, estimate: LoadEstimate): string[] {
  const scale = target.endsWith("_bytes") ? 1024 ** 2 : 1;
  const unit = target.endsWith("_bytes") ? "MiB" : estimate.unit;
