 ...
 * ClawTune sidecar keeps per-repo learning under that key in the KB "repo"
 ...
 * Resolution priority:
 *
 *   1. `CLAWTUNE_REPO_KEY` env                 — explicit override; swe-rebench
...
export function resolveRepoKey(
  ...
  const fromEnv = (process.env.CLAWTUNE_REPO_KEY ?? "").trim();
  if (fromEnv) {
    return fromEnv;
  }
